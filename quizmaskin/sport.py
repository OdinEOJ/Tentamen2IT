...
def sendSpørsmålTilFirebase(questions):
    for index, question in enumerate(questions, start=1):
        sport_id = f"sport{index}"
        try:
            doc_ref = db.collection("sport").document(sport_id).set(question)
